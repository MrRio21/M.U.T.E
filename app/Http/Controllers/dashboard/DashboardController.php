...
public function allRequests(Hotel $id,Order $order){
 ...
 ->where('rooms.hotel_id', '=', $id->id)
   ->get();
    $data2= DB::table("regular_booked_room")
    ->select('regular_booked_room.check_in','regular_booked_room.check_out' ,'regular_booked_room.id as id' , 'hotels.id','regular_booked_room.status as room_status' ,'regular_booked_room.room_id','rooms.price' ,'hotels.name as hotel_name','users.name','rooms.type as type')
    ->join('rooms','regular_booked_room.room_id','=','rooms.id')
    ->join('hotels','rooms.hotel_id', '=', 'hotels.id')
    ->join('users','regular_booked_room.user_id', '=', 'users.id')
...
// dd($allReq);
    ...
    // dd($orderedRooms);
    return view('dashboardHotelOwner.allRequests',['requests'=>$data,'data2'=>$data2]);
}
